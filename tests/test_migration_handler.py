...
from connect.models import Model, Request
...
def test_needsMigration():
    ...
    requests_no_migration = Model.parseArray(Request, response_no_migration)
    assert isinstance(requests_no_migration, Collection)
    assert requests_no_migration.length() == 1
    assert isinstance(requests_no_migration.get(0), Request)
    assert not requests_no_migration.get(0).needsMigration(handler.migration_key)

    # Migration needed
    response_migration = _load_str('request.migrate.valid.json')
    request = Model.parse(Request, response_migration)
    assert isinstance(request, Request)
    assert request.needsMigration(handler.migration_key)

@patch('connect.Logger.info')
def test_no_migration(info_mock):
    # type: (Mock) -> None
    response = _load_str('response.json')
    requests = Model.parseArray(Request, response)
...
def test_migration_skip_all(debug_mock,info_mock):
    ...
    for i, _ in enumerate(request_out.asset.params.toArray()):
        assert request.asset.params.get(i).id == request_out.asset.params.get(i).id
        assert request.asset.params.get(i).value == request_out.asset.params.get(i).value

@patch('connect.Logger.error')
@patch('connect.Logger.debug')
@patch('connect.Logger.info')
...
def test_migration_transform_manual_fail(info_mock, debug_mock, error_mock):
    ...
    info_mock.assert_has_calls([
        call('[MIGRATION::PR-7001-1234-5678] Running migration operations for request PR-7001-1234-5678'),
        call('[MIGRATION::PR-7001-1234-5678] Running transformation for parameter email'),
